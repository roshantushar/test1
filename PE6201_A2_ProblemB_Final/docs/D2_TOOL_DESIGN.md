...
overhead), or an estimated 24,564 wasted tokens across one full 45-case
evaluation pass, for two tools the reference solver never calls on any of
the 45 real cases. This is the concrete version of D6's "tool block size"
...
correctness. **Current, final measurement, on the redesigned 45-case,
107-trial, 69%-negative evaluation set** (`results/descriptors/comparison.json`):
v1 (deliberately worse rules, answer format and descriptors) scores
**27.1% (29/107)**; v2 scores **100% (107/107)**. v1's completed
...
**Case subset:** the 14 `book` cases (1 trial) + the 3
`no_slot_in_window` escalate cases (3 trials) = 23 trials per version —
the only cases in the 45-case set that call `get_clinic_slots` at all;
re-running the other 28 would only reconfirm they are unaffected, which
the dependency structure already guarantees.

**Status: run live** (`openai/gpt-4o-mini`, 23 trials/version — the 14
`book` cases + the 3 `no_slot_in_window` escalate cases at 3 trials
each). Raw data:
`results/descriptors/single_tool_ablation_get_clinic_slots.json`.

| Metric | Descriptor v1 (weak) | Descriptor v2 (full) |
|---|---|---|
| Pass rate | 100.0% (23/23) | 100.0% (23/23) |
| Avg tokens in | 8,613 | 9,122 |
| Negative-case pass rate | 100.0% (9/9) | 100.0% (9/9) |
| Cost/run (23 trials) | $0.0333 | $0.0351 |
| Avg turns | 4.04 | 4.00 |
...
**This is the falsification case the experiment was explicitly
designed to be able to produce, and it is what actually happened:**
weakening only `get_clinic_slots`' own descriptor, with every other v2
component held fixed (the five-check order, the anti-injection
warning, the trigger/missing schema, and every *other* tool's full
descriptor), produced **no measurable correctness difference** on
`openai/gpt-4o-mini` — it never copied a slot's own `band` label
instead of the assessed one, and never queried before all gates
passed, despite v1's descriptor never stating either constraint. The
model's general capability, reinforced by v2's *other* prompt
components (particularly the explicit five-check ordering rule, which
governs *when* `get_clinic_slots` may be called regardless of what that
tool's own docstring says), carried this specific tool's correctness on
its own. **The one real, measured cost is tokens, not correctness**:
v2's fuller descriptor costs 5.9% more input tokens per trial (9,122 vs
8,613) for zero pass-rate gain on this model — a genuine, if narrow,
data point that descriptor *completeness* and prompt-level *ordering
rules* are not interchangeable, and that for a capable model, the
latter can substitute for the former on tools whose calling
constraints are already covered elsewhere in the prompt. This does not
contradict the whole-prompt v1-vs-v2 result above (27.1% vs 100%) —
that comparison varies the rules, schema, AND every descriptor
together, so its huge gap cannot be attributed to any one tool's
descriptor quality in isolation; this narrower ablation isolates
exactly one variable and finds it, alone, was not the one carrying that
gap for `get_clinic_slots` specifically. A weaker model (see D5(b),
`docs/D5_MODEL_BATTERY.md` — `llama-3.1-8b-instruct` and
`mistral-nemo` both score under 40% overall on the full v2 prompt)
would plausibly show a real gap here; this null result is specific to
a capable model already carrying most of the reasoning load via the
other prompt components.

## D2(c) — sequential vs parallel: output

Dependency rule: only calls whose inputs do not depend on each other's
outputs may share a turn. On `REF-5602`, the only such pair is
`check_referral_criteria` + `lookup_patient` (both depend solely on
`get_referral`'s output); `compute_window` → `get_clinic_slots` →
`book_slot` is a genuine chain and can never be shortened.
...
function, not yet a live measurement — see STATUS.md):
...
**Parallelisation did not reduce correctness** — both arms reach
identical decision, identical booked slot (`OPH-C2`, 2026-10-14, 11:20),
identical tool calls; only the turn grouping differs. **Result: 16.7%
fewer turns, an estimated 23% fewer tokens, zero wasted calls** — the
saving is exactly the one independent pair the dependency rule permits,
no more and no less; a dependency chain (the remaining 4 calls) cannot be
shortened by running things at once, which is why the saving is real but
