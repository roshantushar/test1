...
def _live_block():
    ...
    real v1-vs-v2 comparison (D2b) on the IDENTICAL 107-trial, current,
    negative-heavy eval set - both live, both measured, so the "is the
    ...
    return {
        ...
        "MEASUREMENT_STATUS": "REAL live measurement, one model, on the "
                              "current 45-case/107-trial, 69%-negative "
                              "evaluation set (D5b is still one model, not "
                              "the full family comparison).",
        "v1": v1,
        "v2": v2,
        "note": "v2: P=%.4f over a MEDIAN of %d turns -> s=%.4f per step. "
                "v1: P=%.4f over a MEDIAN of %d turns -> s=%.4f per step. "
                "v1 needs MORE turns (median %d vs %d) AND has a much LOWER "
                "P - the worse prompt is not merely slower, it is both "
                "slower and less reliable per step, which rules out 'v1 just "
                "needs more turns to get there' as an explanation. The "
                "actual cause (see results/descriptors/comparison.json) is "
                "that v1 usually reaches the right underlying decision but "
                "cannot supply the exact trigger/missing label the code "
                "check requires - a LABELLING/SCHEMA failure, not a "
                "reasoning-depth failure - which is why extending v1's turn "
                "budget would not close this gap. See docs/D0_AGENT_"
                "JUSTIFICATION.md's 'too many turns vs low per-step "
                ...
                % (v2["P_overall_success"], v2["T_median_turns"], v2["s_per_step_at_median_T"],
                   ...
                   v1["T_median_turns"], v2["T_median_turns"]),
    }
