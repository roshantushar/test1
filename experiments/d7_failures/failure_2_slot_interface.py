...
def main():
    ...
    out = {
        ...
        "why_the_other_two_layers_are_the_wrong_place": (
            "CODE LAYER: the step cap and budget ceiling bound RUN LENGTH; "
            "this run is short (5 turns) and cheap, so neither would ever "
            "fire - they cannot detect a wrong but confidently-reached "
            "answer. Action de-duplication does not apply either: nothing "
            "is repeated. What is wrong is a VALUE (the wrong band reaching "
            "book_slot), and no amount of turn-counting or repeat-detection "
            "looks at argument values at all. PROMPT LAYER: a sentence "
            "asking the calling logic to double-check that band always "
            "comes from check_referral_criteria's assessed value, not a "
            "slot row's own label, is exactly the unreliable fix this "
            "project rejects elsewhere too (see the v1-vs-v2 prompt "
            "comparison, D5(b): a less precisely stated rule scored 41.1% "
            "where the same rule, structurally reinforced, scored 100%) - "
            "a sentence can be forgotten, skipped or misread on any given "
            "run; a function signature that no longer accepts a wrong band "
            "cannot be. TOOL INTERFACE (the fix taken): making band "
            "mandatory AND filtered, so a wrong-band row can never be "
            "returned in the first place - the same reasoning that made "
            "band a required argument on get_clinic_slots and "
            "specialty+band required, cross-checked arguments on book_slot "
