...
# Monthly limit per caller (per REFERRING CLINIC - the natural caller
# identity for Problem B): at D6's assumed 4,000 referrals/month across
# the 14 distinct referring clinics in this dataset, average volume per
# clinic is ~286/month; 500 is set well above that average (so it never
# fires on ordinary use) but low enough to catch a genuine runaway
# pattern - a misconfigured integration re-submitting the same batch,
# or abuse - within the same month it happens, not after.
MAX_MONTHLY_REQUESTS_PER_CALLER = 500
